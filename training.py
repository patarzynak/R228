...
tf.app.flags.DEFINE_string("save_dir", "../tmp_def/", "Directory for saving model."
...
tf.app.flags.DEFINE_string("out_form", "cosine", "Eval output:")
...
def train_network(model, num_epochs, batch_size, data_dir, save_dir,
  ...
  with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
    ...
    for idx, epoch in enumerate(
      ...
      with open("losses_"+m+str(bs)+str(es)+"dump.txt", "a") as lossfile:
          lossfile.write(str(training_losses))
      training_losses =[]
...
def evaluate_model(sess, data_dir, input_node, target_node, prediction,
  ...
  dev_data = gen_epochs(data_path=data_dir, total_epochs=1,
          #batch_size=FLAGS.batch_size, vocab_size=FLAGS.vocab_size, phase='dev')
          batch_size=200, vocab_size=FLAGS.vocab_size, phase='dev')
  print("Starting evaluation.")
  for step, (data_x, data_y) in enumerate(next(dev_data)):
      print("Step: ",step)
      pred = sess.run(prediction, feed_dict={input_node: data_x, target_node: data_y})
      
      if out_form == "cosine":
          c_dists = 1 - np.squeeze(dist.cdist(pred, embs, metric="cosine"))
          c_dists = np.nan_to_num(c_dists)
          rank_arr = np.argsort(c_dists)[::1]
          #print("rank shape ", rank_arr.shape)
      else:#softmax (?)
          rank_arr =  np.squeeze(pred)[2:].argsort()[::1] + 2
          
      #print(data_y.shape)
      rank_arr = np.fliplr(rank_arr)
      label_arr = data_y[:,np.newaxis]
      ranks = np.where(rank_arr == label_arr)[1]
      print(ranks)
      #for idx in range(200):
      #    print('Correct word: %s ranked: %d' % (rev_vocab[data_y[idx]], ranks[idx]))

      #median = np.median(ranks)
      #print("Median: ", median)
      all_ranks += ranks.tolist()

  final_median = np.median(np.asarray(all_ranks))
...
'''
  ...
  epoch = next(gen_epochs(data_dir, num_epochs, batch_size, vocab_size, phase="dev"))
  dev_loss, mean_ranking = 0, 0
  print("Evaluating")
  for step, (gloss, head) in enumerate(epoch):
      predictions, _dev_loss = sess.run([prediction, loss], feed_dict={input_node:gloss, target_node: head})
      dev_loss += _dev_loss
  ...
  if out_form == "softmax":
      ...
      ranked_vocab = np.fliplr(sims.argsort(1))
      head_array = np.expand_dims(head, axis=1)
      answer_rankings = np.where(ranked_vocab == head_array)[1]
      _mean_ranking = np.mean(answer_rankings)
      mean_ranking += _mean_ranking
  avg_loss = dev_loss / float(step+1)
  avg_mean_ranking = mean_ranking / float(step+1)
  print("Average mean ranking: ", avg_mean_ranking)
  '''
...
def restore_model(sess, save_dir, vocab_file, out_form):
  ...
  if out_form == "softmax":
    ...
    #predictions = graph.get_tensor_by_name("fully_connected/Sigmoid:0")
